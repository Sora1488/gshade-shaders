...
temp = tex2Dlod(SamplerOcclusion1, float4(texcoord.xy + axis * float2((1.0 / 1920), (1.0 / 1080)) * r, 0.0, 0.0));
...
temp = tex2Dlod(SamplerOcclusion2, float4(texcoord.xy + axis * float2((1.0 / 1920), (1.0 / 1080)) * r, 0.0, 0.0));
float weight = AO_BLUR_STEPS-abs(r);
weight *= saturate(1.0 - (1000.0 * AO_SHARPNESS) * abs(temp.w - base.w));
sum += temp.x * weight;
totalweight += weight;
...
const float2 sample_vec_divisor = float2(tan(0.5f*radians(75)) / (float)(1.0 / 1080) * (float)(1.0 / 1920), tan(0.5f*radians(75))) * depth / (fSSGISamplingRange * float2((1.0 / 1920), (1.0 / 1080)).xy);
...
temp = tex2Dlod(SamplerOcclusion1, float4(texcoord.xy + axis * float2((1.0 / 1920), (1.0 / 1080)) * r, 0.0, 0.0));
const float tempdepth = ReShade::GetLinearizedDepth(texcoord + axis * float2((1.0 / 1920), (1.0 / 1080)) * r).x;
float tempkey;
if( AO_SHARPNESS_DETECT == 0)
tempkey = tempdepth;
else
tempkey = dot(GetNormalFromDepth(tempdepth, texcoord.xy + axis * float2((1.0 / 1920), (1.0 / 1080)) * r).xyz,0.333)*0.1;
#line 1147
float weight = AO_BLUR_STEPS-abs(r);
